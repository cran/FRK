...
| \Sigmamat_Z^{(l)}  | &= | (\Kmat^{(l)})^{-1} + \Smat_Z^\top(\Dmat_Z^{(l)})^{-1} \Smat_Z | |\Kmat^{(l)} | |\Dmat_Z^{(l)} |,%\label{eq:determinant}
\end{align*}
...
\Sigmamat_W &\equiv (\Pimat^\top \Cmat_Z^\top \Sigmamat_\epsilon^{-1} \Cmat_Z \Pimat + \Lambdamat^{-1})^{-1},\\% \label{eq:Winv}
\end{align*}
