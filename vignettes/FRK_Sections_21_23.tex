...
In \pkg{FRK}, we assume that $\etab$ is an $r$-dimensional Gaussian vector with mean zero and $r \times r$ covariance matrix $\Kmat$, and estimation of $\Kmat$ is based on likelihood methods. If some structure is imposed on $\var(\etab)$ in terms of parameters $\varthetab$, then $\Kmat = \Kmat_\circ(\varthetab)$ and $\varthetab$ needs to be estimated. Frequently, the resolution of the BAUs is sufficiently fine, and the basis functions are sufficiently smooth, so that $\Smat$ can be approximated:
...
\end{align*}
In practice, it is not always possible for each $B_i$ to include entire BAUs. For simplicity, in \pkg{FRK} we assume that the observation footprint overlaps a BAU if and only if the BAU centroid lies within the footprint. Frequently, point-referenced data is also supplied. In this case each data point is attributed to a specific BAU and it is possible to have multiple observations of the same BAU.
...
\end{equation*}
where recall that $m$ is the dimension of the data vector $\Zvec$ and $\alphab^{(l+1)}$ is, in this special case, the ordinary-least-squares estimate given $\muvec_\eta^{(l)}$. These simplifications are used by \pkg{FRK} when possible.
...
| \Sigmamat_Z^{(l)}  | &= | (\Kmat^{-1})^{(l)} + \Smat_Z^\top(\Dmat_Z^{-1})^{(l)} \Smat_Z | |\Kmat^{(l)} | |\Dmat_Z^{(l)} |,%\label{eq:determinant}
\end{align*}
